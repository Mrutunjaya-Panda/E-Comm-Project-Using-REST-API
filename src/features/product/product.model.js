...
export default class ProductModel {
  ...
  //since now we are using mongodb database to store the products, so we will not be using the static methods in the product model to perform operations on the products collection in the database, instead we will be creating a separate repository class for products, which will contain all the methods to perform operations on the products collection in the database, and then we will be using that repository class in our product controller to handle the requests and responses related to products, let's see how to do that in our product.repository.js file.
  //therefore we commented these.
