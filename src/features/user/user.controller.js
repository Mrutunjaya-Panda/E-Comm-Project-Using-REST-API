...
export default class UserController {
    ...
    try {
      ...
      if (existingUser) {
        res
          .status(400)
          .send(
            "User already exists with the same email. Please use a different email to signup.",
          );
        return;
      }
  ...
  //we will create a method to reset password for the user, which will be used when user forgets his/her password and wants to reset it,
  // we will send a reset password link to the user's email, and when user clicks on that link, we will redirect him/her to a page where he/she can enter new password and confirm it,
  // and then we will update the password in the database with the new password entered by the user.
  async resetPassword(req, res, next) {
    try {
      const { newPassword, confirmPassword } = req.body;
      const userId = req.userId; // we can get the userId from the request object, which is set in the jwtAuth middleware after verifying the token, so that we can identify the user and update his/her password in the database.
    ...
    //   logger.error(
    //     "Error occurred while resetting the password for the user: " +
    //       err.message,
    //   );
      console.error(
        "Error occurred while resetting the password for the user:",
